...
**重要要求（请严格遵守）：**
1. **只返回纯 JSON 格式，不要包含任何 Markdown 代码块标记（如 ```json 或 ```）**
2. **不要包含任何注释（如 // 或 /* */）**
3. **不要包含任何解释性文本、说明文字或其他内容**
4. **直接返回 JSON 对象，从 {{ 开始，到 }} 结束**
...
4. **绝对不要添加注释、不要添加代码块标记、不要添加任何解释性文字**

**现在请直接返回执行计划的 JSON（只返回 JSON，不要其他任何内容）：**"""
