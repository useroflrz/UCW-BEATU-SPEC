...
| **分享（竖屏）** | - 点击竖屏底部分享按钮 → 弹出底部白色圆角面板，展示“生成分享图、链接分享”两个入口；<br>- **生成分享图**：<br>  &nbsp;&nbsp;① 截取当前视频封面画面；<br>  &nbsp;&nbsp;② 使用 ZXing 生成指向视频播放地址的二维码；<br>  &nbsp;&nbsp;③ 用 Canvas 合成竖版海报（上方为封面大图，下方白色信息卡片：标题、作者、二维码+“扫码查看视频”文案）；<br>  &nbsp;&nbsp;④ 通过 `FileProvider` 写入 `cacheDir/share_images` 并调起系统图片分享；<br>- **链接分享**：构造 `ACTION_SEND` 文本 Intent，内容为“标题 + 播放链接”，并在点击时调用后端分享上报接口。分享成功/失败不阻塞系统分享流程。 |
| **分享（横屏）** | - 横屏模式右侧竖向控制条中包含“分享”按钮；<br>- 点击后在屏幕右侧贴边弹出白色圆角浮窗（与原型一致），上下展示“复制链接 / 保存图片”两项；<br>- 复制链接：构造 `ACTION_SEND` 文本 Intent，内容为“标题 + 播放链接”；<br>- 保存图片：复用竖屏的海报生成能力，生成“封面+二维码”图片后调用系统图片分享；<br>- 所有分享入口都会调用统一的 `ShareVideoUseCase`，通知后端将当前视频的 `share_count` 自增，并在前端对当前卡片的分享计数做一次乐观 +1 更新。 |
...
| **亮度调节** | 右侧亮度按钮 **长按+上/下滑** → 亮度增/减；亮度条在右侧按钮原位置浮现：<br>① 长按按钮一段时间后，按钮隐藏，原位置显示椭圆形亮度条（太阳图标 + 百分比）；<br>② 手指保持按住并上下滑动时，亮度条始终可见，仅根据滑动距离实时调整亮度和填充高度（约 1/3 屏幕高度的滑动即可从暗到亮）；<br>③ 调节期间，单击/双击、进度滑动、音量手势和其他按钮点击全部被屏蔽，防止误触；<br>④ 松手后解除亮度锁定，但亮度条继续保留约 1 秒后自动收起并恢复按钮。 |
| **音量调节** | 右侧音量按钮 **长按+上/下滑** → 音量增/减；交互与亮度一致：<br>① 长按音量按钮后，按钮隐藏，原位置显示椭圆形音量条（喇叭图标 + 百分比），锁屏按钮在其上方保持位置不变；<br>② 手指保持按住并上下滑动时，音量条始终可见，根据滑动距离实时调整系统媒体音量和填充高度（内部以完整音量格为最小步长，视觉按 0~100% 显示）；<br>③ 调节期间屏幕其他手势与按钮点击全部被屏蔽，仅保留音量调节；<br>④ 松手后解除音量锁定，音量条继续保留约 1 秒后自动收起并恢复按钮。 |
